...
class RelativePoseDataset(Dataset):
    def __init__(self, num_past_timestamps=100, num_future_timestamps= 100, relative_directory='../../expert_ML/predict_dataset') -> None:
        """
        RETURN TWO THINGS:
    ...
    def _read_from_pickle(self, path):
        with open(path, 'rb') as file:
            ...
            actual_rel_pose = data[0][3] # This is a list (Ntimesteps) which containts 4x4 array (transformation matrix)
            ...
            # For visualization, convert matrices to pose representations
            actual_pose_list = [self.matrix_to_pose(matrix) for matrix in actual_rel_pose]
            offset_pose_list = [self.matrix_to_pose(matrix) for matrix in offset_applied_pose]

            # Plot the comparison
            # self._plot_pose_comparison(actual_pose_list, offset_pose_list)

            wrench_data = np.array(data[0][5])  # This is N timestep x 6

            noisy_wrench = []
            for wrench in wrench_data:
                noise = np.random.normal(0, 0.5, wrench.shape)
                noisy_wrench.append(wrench + noise)

            trajectory = {
                'obs_wrench': noisy_wrench,  # List of N timesteps x 7 (x, y, z, qx, qy, qz, qw)
                'obs_pose': offset_applied_pose  # List of N timesteps x 6 (assuming wrench data has 6 components)
            }
            self.trajectories.append(trajectory)
            self.filenames.append(path)

    def _plot_pose_comparison(self, actual_pose_list, offset_pose_list):
        # Convert lists to numpy arrays for easier manipulation
        actual_pose_array = np.array(actual_pose_list)
        offset_pose_array = np.array(offset_pose_list)

        # Extract translation and quaternion components for visualization
        actual_translation = actual_pose_array[:, :3]
        offset_translation = offset_pose_array[:, :3]
        actual_quaternion = actual_pose_array[:, 3:]
        offset_quaternion = offset_pose_array[:, 3:]

        # Plot translations
        plt.figure(figsize=(12, 6))

        plt.subplot(2, 1, 1)
        plt.plot(actual_translation[:, 0], label='Actual Translation X')
        plt.plot(offset_translation[:, 0], label='Offset Translation X', linestyle='--')
        plt.plot(actual_translation[:, 1], label='Actual Translation Y')
        plt.plot(offset_translation[:, 1], label='Offset Translation Y', linestyle='--')
        plt.plot(actual_translation[:, 2], label='Actual Translation Z')
        plt.plot(offset_translation[:, 2], label='Offset Translation Z', linestyle='--')
        plt.legend()
        plt.title('Translation Comparison')

        # Plot quaternions
        plt.subplot(2, 1, 2)
        plt.plot(actual_quaternion[:, 0], label='Actual Quaternion X')
        plt.plot(offset_quaternion[:, 0], label='Offset Quaternion X', linestyle='--')
        plt.plot(actual_quaternion[:, 1], label='Actual Quaternion Y')
        plt.plot(offset_quaternion[:, 1], label='Offset Quaternion Y', linestyle='--')
        plt.plot(actual_quaternion[:, 2], label='Actual Quaternion Z')
        plt.plot(offset_quaternion[:, 2], label='Offset Quaternion Z', linestyle='--')
        plt.plot(actual_quaternion[:, 3], label='Actual Quaternion W')
        plt.plot(offset_quaternion[:, 3], label='Offset Quaternion W', linestyle='--')
        plt.legend()
    ...
    def __getitem__(self, idx):
        ...
        if start_idx < 0:
            ...
            obs_wrench_start = np.tile(data['obs_wrench'][0], (num_missing,1,1))
            obs_wrench_start = np.squeeze(obs_wrench_start, axis=1)
            obs_wrench_rest = np.array(data['obs_wrench'][:local_idx+1])
            obs_wrench = np.concatenate((obs_wrench_start,obs_wrench_rest), axis=0)

        else:
            obs_mat = np.array(data['obs_pose'][start_idx:local_idx + 1]) # This is 20 x 4 x 4 where 4x4 is the transformation matrix
            obs_wrench = np.array(data['obs_wrench'][start_idx:local_idx+1])
            # print(obs_wrench.shape)
        # Change to pose use matrix to pose
        obs_pose = [self.matrix_to_pose(matrix) for matrix in obs_mat]
        obs = np.hstack((obs_pose,obs_wrench))
