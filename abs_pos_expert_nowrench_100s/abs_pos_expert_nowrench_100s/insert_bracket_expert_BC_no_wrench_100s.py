...
class DataCollector:
    def __init__(
        ...
        self.model_path = "/home/davintj/ws_raise/src/metalman/metalman_core/src/metalman_core/insertion/RL/abs_pos_expert_nowrench_100s/runs_FFBC_nowrench_epoch_142.pth" ## EPOCH 142 NEEDS REALLY HIGH GAIN 7, 10
        self.model = FFBC()
    ...
    def expert_insertion(self):
        ...
        def loop(timer_event):
            ...
            if self.state == State.SWITCH_CONTROLLER:
                ...
                if target_distance < 0.002 and rotation_angle<0.002:
                    self.call_FT_zero_calib(self.zero_ftsensor_service)
                    self.call_FT_zero_calib(self.zero_ftsensor_service)
                    current_pose = self.get_current_end_effector_pose()
                    rel_curr_to_goal = np.linalg.inv(self.goal_pose) @ current_pose  # 4x4 matrix
