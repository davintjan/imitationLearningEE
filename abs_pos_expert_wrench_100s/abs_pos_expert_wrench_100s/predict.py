...
def main():
    model = FFBC()
    model.load_state_dict(torch.load("runs_FFBC_model_epoch_122.pth"))
    model.eval()
    ...
    # print(actual_labels)
    """ 
    Actual labels is a list timestep (1227) of ndarray (1,130) -> 130 is just action over the next 10 timesteps flattened. So it is x,y,z,qx,qy,qz,qw, wrench over 10 timesteps
    Actual labels is a list timestep (1227) of ndarray (1,130) -> 130 is just action over the next 10 timesteps flattened. So it is x,y,z,qx,qy,qz,qw, wrench over 10 timesteps
    Plot actual labels 
    """
