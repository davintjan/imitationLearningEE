...
import torch
from metalman_core.insertion.RL.abs_pos_expert_wrench_100s.Model import *
...
class DataCollector:
    def __init__(
        ...
        # Init solver
        self.solver_reconfiguration_client = dynamic_reconfigure.client.Client("fr20/cartesian_motion_controller/solver", timeout=10)
        self.move_away_solver = [0.1, 40, 0.01]
        # self.BC_solver = [0.05, 40, 0.01]
        self.BC_solver = [0.05, 40, 0.01]
        self.fwd_dyn_reconfiguration_client = dynamic_reconfigure.client.Client("fr20/cartesian_motion_controller/solver/forward_dynamics", timeout=10)
        ...
        self.model_path = "/home/davintj/ws_raise/src/metalman/metalman_core/src/metalman_core/insertion/RL/abs_pos_expert_wrench_100s/runs_FFBC_wrench_noise_epoch_70.pth"
        self.model = FFBC()
    ...
    def solver_reconfig_fn(self, solver):
        self.solver_reconfiguration_client.update_configuration(
            {"error_scale": solver[0], "iterations": solver[1]}
        )
        self.fwd_dyn_reconfiguration_client.update_configuration(
            {"link_mass": solver[2]}
        )
    ...
    def expert_insertion(self):
        ...
        input = np.hstack((rel_curr_to_goal_quat,self.filtered_wrench))
        self.model_input = np.tile(input, (100, 1)).flatten()
        ...
        def loop(timer_event):
            ...
            if self.state == State.SWITCH_CONTROLLER:
                ...
                if response == True:
                    ...
                    self.state = self.state.MOVE_AWAY
                    print(f"{self.state} is next!")

            elif self.state == State.MOVE_AWAY:
                self.solver_reconfig_fn(self.move_away_solver)
                current_pose = self.get_current_end_effector_pose()
                target_frame = move_away_pose
                target_distance = np.linalg.norm(current_pose[:3, 3] - target_frame[:3, 3])
                print(target_distance)
                if target_distance < 0.004:
                    self.call_FT_zero_calib(self.zero_ftsensor_service)
                    self.call_FT_zero_calib(self.zero_ftsensor_service)
                ...
                current_pose = self.get_current_end_effector_pose()
                target_frame = initial_pose
                target_distance = np.linalg.norm(current_pose[:3, 3] - target_frame[:3, 3])
                # Calculate the rotation difference
                current_orientation = Rotation.from_matrix(current_pose[:3, :3])
                target_orientation = Rotation.from_matrix(target_frame[:3, :3])
                rotation_diff = target_orientation * current_orientation.inv()
                rotation_angle = rotation_diff.magnitude()
                print(target_distance)
                print(rotation_angle)

                if target_distance < 0.004 and rotation_angle<0.003:
                    self.call_FT_zero_calib(self.zero_ftsensor_service)
                    self.call_FT_zero_calib(self.zero_ftsensor_service)
                    current_pose = self.get_current_end_effector_pose()
                    rel_curr_to_goal = np.linalg.inv(self.goal_pose) @ current_pose  # 4x4 matrix
                    ...
                    input = np.hstack((rel_curr_to_goal_quat, self.filtered_wrench))
                    self.model_input = np.tile(input, (100, 1)).flatten()
                    self.solver_reconfig_fn(self.BC_solver)
                    rospy.sleep(3)
                    self.state = State.BC
                ...
                rel_curr_to_goal_quat = self.matrix_to_pose(rel_curr_to_goal)
                input = np.hstack((rel_curr_to_goal_quat, self.filtered_wrench))
                self.model_input = np.roll(self.model_input, shift=-13)  # Shift left by 7 elements # This might not be the correct way to do this
                self.model_input[-13:] = input
                ...
                actual_rel_poses = np.linalg.inv(self.goal_pose).dot(current_pose)
                ...
                print(abs(self.filtered_wrench[2]))
                print("Z Axis Model", self.model_input[-11])
                print("Actual Rel Z Axis", actual_rel_poses[2,-1])
                if actual_rel_poses[2, -1] > -0.003 or abs(self.filtered_wrench[0]) > 200 or abs(self.filtered_wrench[1] > 200 or abs(self.filtered_wrench[2] > 200)):
                    print("SUCCESSFUL BC INSERTION")
